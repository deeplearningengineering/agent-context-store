...
프로젝트 수준에 적용 시, 프로젝트 최상단 `.cursor/` 폴더에 셋팅합니다.
전역 셋팅을 원할 경우 `~/.cursor/` 폴더에 적용하세요.

### Claude Code에서 사용하기

Claude Code 실행 시 `rules/` 디렉토리의 가이드라인을 컨텍스트로 주입하여, 팀의 특정 라이브러리 활용 방식이나 비즈니스 ��직 패턴을 따르도록 명령합니다.
...
관련 문서: [Cursor - Rules](https://cursor.com/ko/docs/context/rules)
...
관련 문서: [Cursor - Subagents](https://cursor.com/ko/docs/context/subagents)
...
### 3. skills/ (재사용 가능한 작업 절차)
**무엇을 할 수 있는가?**에 대한 도메인 특화 지시서
관련 문서: [Cursor - Skills](https://cursor.com/ko/docs/context/skills)

- **워크플로우 자동화**: 복잡한 태스크를 단계별로 실행하기 위한 시퀀스 및 도구 사용 순서 가이드(예: JSDoc 및 README 자동 생성 템플릿, 테스트 코드 작성 시 반드시 포함되어야 할 Edge Case 체크리스트)
...
1. `rules/` 또는 `skills/`에 새 `.md` 파일 생성
2. 모든 팀원의 IDE에 최신 컨텍스트 동기화
3. 사용 예시: AI 에이전트에게 "이 레포지토리의 rules/00000.md를 읽고 내 코드를 리뷰해줘"라고 요청
